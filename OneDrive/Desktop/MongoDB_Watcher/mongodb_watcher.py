...
# Load LLM (Mistral 7B or any compatible GGUF loaded with Transformers)
@st.cache_resource
def load_model():
...
def generate_query(user_input, area_of_law):
    prompt = f"""
...
if st.button("Search"):
    if not user_input or not area:
        ...
        with st.spinner("Generating query and searching MongoDB..."):
            query = generate_query(user_input, area)
            if query:
